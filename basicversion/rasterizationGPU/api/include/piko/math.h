...
namespace piko {
	...
	__device__ __inline__ int ffsll_cuda(unsigned long long x) {
		if(x == 0)
			return 0;
	...
	__device__ __inline__ int popcll_cuda(unsigned long long x) {
		int c = 0;
		for(c = 0; x; x >>= 1)
			c += x & 1;
		return c;
	...
	__device__ __inline__ unsigned   	f32_to_u32_sat_rmi_cuda	(float a)                 							{ unsigned v; asm __volatile__("cvt.rmi.sat.u32.f32 %0, %1;" : "=r"(v) : "f"(a)); return v; }
	...
	__device__ __inline__ unsigned   	prmt_cuda								(unsigned a, unsigned b, unsigned c)   	{ unsigned v; asm __volatile__("prmt.b32 %0, %1, %2, %3;" : "=r"(v) : "r"(a), "r"(b), "r"(c)); return v; }
	inline float   			fma_rm              (float a, float b, float c)   					{ float v; asm __volatile__("fma.rm.f32 %0, %1, %2, %3;" : "=f"(v) : "f"(a), "f"(b), "f"(c)); return v; }
	__device__ __inline__ float   			fma_rm_cuda              (float a, float b, float c)   					{ float v; asm __volatile__("fma.rm.f32 %0, %1, %2, %3;" : "=f"(v) : "f"(a), "f"(b), "f"(c)); return v; }
	...
	{
		// 11 instructions: 4*FFMA, 4*F2I, 3*PRMT
		unsigned x = f32_to_u32_sat_rmi_cuda(fma_rm_cuda(color.x, (1 << 24) * 255.0f, (1 << 24) * 0.5f));
		unsigned y = f32_to_u32_sat_rmi_cuda(fma_rm_cuda(color.y, (1 << 24) * 255.0f, (1 << 24) * 0.5f));
		unsigned z = f32_to_u32_sat_rmi_cuda(fma_rm_cuda(color.z, (1 << 24) * 255.0f, (1 << 24) * 0.5f));
		unsigned w = f32_to_u32_sat_rmi_cuda(fma_rm_cuda(1.0f,    (1 << 24) * 255.0f, (1 << 24) * 0.5f));
		return prmt_cuda(prmt_cuda(x, y, 0x0073), prmt_cuda(z, w, 0x0073), 0x5410);
	}
	...
	__device__ __inline__ int 					imad_cuda 								(int a, int b, int c)										{ return (a*b+c); }
	...
	__device__ __inline__ float   			rcp_approx_cuda					(float a)                 							{ return 1.0f/a; }
	...
	__device__ __inline__ int 					imad_cuda 								(int a, int b, int c)										{ return __BACKEND_UNDEFINED_INT__();}
	...
	__device__ __inline__ float   		rcp_approx_cuda					(float a)                 							{ return __BACKEND_UNDEFINED_FLOAT__();}
	...
	__device__ __inline__ unsigned toABGR_cuda(cvec3f color)		{ return __BACKEND_UNDEFINED_INT__(); }
