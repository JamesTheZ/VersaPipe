...
__device__ int rystate[1024*1024];
__device__ int tmpNums[1024*1024];
__device__ unsigned int dppos;

	__global__
void splitDP(piko_patch *primArray, 
		piko_patch *newPrimArray, piko_patch *dicePrimArray, 
		int *accNumPrim, int *numPrimDice, int numPrims,
		piko_upoly *dicePoly, int *numPrimPoly)
{
	...
	if(gid < numPrims)
	{
		...
		unsigned int pos = atomicInc(&dppos, 1024*1024);
		
		tmpNums[pos] = -1;
		rystate[pos] = -1;

		SplitStage::process_new(prim, newPrimArray, accNumPrim, 
				dicePrimArray, numPrimDice, &tmpNums[pos], &rystate[pos]);

		int posPrim = tmpNums[pos];
		int state = rystate[pos];

		if(state == 0)
		{
			cudaStream_t s;
			cudaStreamCreateWithFlags(&s, cudaStreamNonBlocking);
			splitDP<<<1, 2, 0, s>>>(&newPrimArray[posPrim],
					newPrimArray, dicePrimArray, 
					accNumPrim, numPrimDice, 2,
					dicePoly, numPrimPoly);
			cudaStreamDestroy(s);
		}
		else if(state == 1)
		{
			piko_patch prim = dicePrimArray[posPrim];
...
void ReyesPipe::run_single()
{
	...
	checkCudaErrors(cudaMemcpy(accNumPrim, &numPrims, sizeof(int), cudaMemcpyHostToDevice));
	...
	splitDP<<<blockDim_x, threadDim>>>(primArray, 
		primArray, dicePrimArray, 
		accNumPrim, numPrimDice, numPrims,
		dicePoly, numPolyShade_d);
