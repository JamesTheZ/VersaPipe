...
struct cvec2u { 
	unsigned int x, y; 
	__host__ __device__ cvec2u() {}
	__host__ __device__ cvec2u (unsigned int tx, unsigned int ty) :x(tx), y (ty) {}
	__host__ __device__ cvec2u& operator= (cvec2u &t) {
		x = t.x;
		y = t.y;
		return *this;
	}
...
struct Pixel : public Primitive
{
	...
	//__host__ __device__ Pixel()  {color = 0; pos.x = 0; pos.y = 0;}

	__host__ __device__ void setColor(float r, float g, float b, float a) {
