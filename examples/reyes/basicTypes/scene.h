    ...
    for(uint bi = 0; bi < _bezmeshes.size(); bi++){
      ...
      for(uint pi=0; pi < _bezmeshes[bi]->patches().size(); pi++){
        patch &p = _bezmeshes[bi]->patches()[pi];
            ...
            // printf("original value %d: flattened patch: %f %f %f %f\n",flatPidx, p.cp[j][k].x,
            //                                          p.cp[j][k].y,
            //                                          p.cp[j][k].z,
            //   p.cp[j][k].w);

            // printf("saved value %d: flattened patch: %f %f %f %f\n",flatPidx, _flatPatches[flatPidx*16 + j*4 + k].x,
            //                                          _flatPatches[flatPidx*16 + j*4 + k].y,
            //                                          _flatPatches[flatPidx*16 + j*4 + k].z,
            //   _flatPatches[flatPidx*16 + j*4 + k].w);
			    }
			  }
        flatPidx++;
      } // next patch
